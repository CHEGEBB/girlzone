...
export default function SubscriptionPlanForm({ plan, isEdit = false }: SubscriptionPlanFormProps) {
  ...
  const validateForm = () => {
    ...
    if (formData.monthly_bonus_tokens !== undefined && formData.monthly_bonus_tokens < 0) {
      newErrors.monthly_bonus_tokens = "Monthly bonus tokens must be a positive number"
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <Card className="bg-[#141414] border-[#252525]">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="monthly_bonus_tokens">Monthly Bonus Tokens</Label>
            <Input
              id="monthly_bonus_tokens"
              name="monthly_bonus_tokens"
              type="number"
              min="0"
              value={formData.monthly_bonus_tokens || 0}
              onChange={handleNumberChange}
              placeholder="e.g., 100"
              className={errors.monthly_bonus_tokens ? "border-red-500" : ""}
            />
            <p className="text-sm text-muted-foreground">Tokens granted automatically each month</p>
            {errors.monthly_bonus_tokens && <p className="text-sm text-red-500">{errors.monthly_bonus_tokens}</p>}
          </div>
        </CardContent>
      </Card>
