...
import { Home, MessageSquare, ImageIcon, Settings, Crown, ChevronLeft, LogOut, User, FolderHeart, DollarSign } from "lucide-react"
...
export default function Sidebar() {
  ...
  // Fetch subscriptions enabled setting and monetization status
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        // Check subscriptions enabled
        const subscriptionsResponse = await fetch("/api/settings/subscriptions_enabled");
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="p-4 border-b border-[#252525] flex items-center justify-between">
          <Link href="/" className="flex items-center">
            ...
            <span className="text-xl font-bold text-[#1111FF]">{settings.siteSuffix}</span>
          </Link>
          ...
          <nav className="space-y-2 px-2">
            ...
            {monetizationEnabled && (
              <Link href="/monetization">
                <Button
                  variant="ghost"
                  className={`w-full justify-start ${pathname?.startsWith("/monetization") ? "bg-[#252525]" : ""}`}
                >
            ...
            {monetizationEnabled && (
              <Link href="/premium">
                ...
                >
                  <Crown className="mr-2 h-5 w-5 text-[#1111FF]" />
          ...
          {user ? (
            ...
            <div className="space-y-2">
              ...
              <Link href="/signup" className="block">
                <Button className="w-full bg-[#1111FF] hover:bg-[#0e0ecc]">Sign Up</Button>
              </Link>
            </div>
          )}
        </div>
