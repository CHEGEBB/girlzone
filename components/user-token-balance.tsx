...
export function UserTokenBalance({
    ...
    return (
        <Link href="/premium?tab=tokens" className={`flex items-center gap-1 hover:opacity-80 transition-opacity ${className}`}>
            {showIcon && <Coins size={iconSize} className="text-[#1111FF]" />}
            <span className={`font-medium ${textSizeClass}`}>{balance}</span>
        </Link>
    )
}
