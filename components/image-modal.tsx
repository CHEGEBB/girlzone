  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl p-0 bg-[#0A0A0A] border-[#252525] rounded-lg">
        ...
        {/* Action buttons */}
        <div className={`flex justify-center ${isMobile ? 'gap-2 p-3 w-full' : 'gap-4 p-4'} bg-[#1A1A1A] rounded-b-lg`}>
          <Button
            variant="outline"
            onClick={() => onDownload(images[currentIndex], currentIndex)}
            className={`bg-[#252525] border-[#333333] hover:bg-[#333333] ${isMobile ? 'flex-1 text-[10px] px-1 h-8' : ''}`}
          >
            <Download className={`h-3.5 w-3.5 ${isMobile ? 'mr-1' : 'mr-2'}`} />
            Download
          </Button>
          <Button
            variant="outline"
            ...
            className={`bg-[#252525] border-[#333333] hover:bg-[#333333] ${isMobile ? 'flex-1 text-[10px] px-1 h-8' : ''}`}
          >
            <Share2 className={`h-3.5 w-3.5 ${isMobile ? 'mr-1' : 'mr-2'}`} />
            Share
          </Button>
          {onSave && (
            <Button
            ...
            >
              {savingIndex === currentIndex ? (
                <Loader2 className={`h-3.5 w-3.5 animate-spin ${isMobile ? 'mr-1' : 'mr-2'}`} />
              ) : (
                <Save className={`h-3.5 w-3.5 ${isMobile ? 'mr-1' : 'mr-2'}`} />
              )}
              {isMobile ? 'Save' : 'Save'}
            </Button>
          )}
