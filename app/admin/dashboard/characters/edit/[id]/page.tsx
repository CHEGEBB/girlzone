...
export default function EditCharacterPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    if (character) {
      setFormData({
        ...
        disableVideoHover: character.disableVideoHover || false,
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Upload to Bunny.net via our API
      ...
      // Update form data with the Bunny.net CDN URL
      setFormData((prev) => ({ ...prev, image: result.url }))

      console.log('Image uploaded successfully to Bunny.net:', result.url)
    } catch (err) {
  ...
  const handleGenerateVideo = async () => {
    if (!formData.image || formData.image.includes("placeholder")) {
      setError("Please upload a character image before generating a video")
      return
    }
    ...
    try {
      ...
      if (!session?.access_token) {
        throw new Error("You must be logged in to generate videos. Please refresh the page and try again.")
      }

      // Use the character's description as the prompt for the video
      const videoPrompt = `A high-quality animation of ${formData.name}. ${formData.description}. Cinematic lighting, detailed background, fluid movement.`
      ...
      if (result.success && result.job_id) {
        setSuccess(`Video generation started! Job ID: ${result.job_id}. The video will be available shortly.`)

        // Wait for generation to complete (polling)
        const checkStatus = async () => {
          try {
            const statusResponse = await fetch(`/api/check-video-generation?job_id=${result.job_id}`)
            if (statusResponse.ok) {
              const statusData = await statusResponse.json()
              if (statusData.status === 'COMPLETED' && statusData.video_url) {
                setFormData(prev => ({ ...prev, videoUrl: statusData.video_url }))
        ...
        <div className="flex-1 overflow-auto">
          ...
          <div className="p-6">
            <div className="bg-[#1A1A1A] rounded-xl p-6 mb-6">
              ...
              {success && (
                <div className="mb-6 p-4 bg-green-900/20 border border-green-800 text-green-300 rounded-lg flex items-center">
                  <div className="h-5 w-5 mr-2 flex-shrink-0 bg-green-500 rounded-full flex items-center justify-center text-green-900">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center">
                        <input
                          id="disableVideoHover"
                          name="disableVideoHover"
                          type="checkbox"
                          checked={formData.disableVideoHover}
                          onChange={(e) => setFormData((prev) => ({ ...prev, disableVideoHover: e.target.checked }))}
                          className="h-4 w-4 rounded border-gray-300 text-[#FF4D8D] focus:ring-[#FF4D8D]"
                        />
                        <label htmlFor="disableVideoHover" className="ml-2 block text-sm text-gray-300 flex items-center">
                          <Play className="h-4 w-4 mr-1 text-gray-400" />
                          Disable Video Hover
                        </label>
                      </div>
                      <p className="text-xs text-gray-400">If checked, the hover video effect will be disabled.</p>
                    </div>
