...
export async function POST(req: NextRequest) {
  try {
    ...
    const tokenCost = 5 // Cost for img2img generation

    // Check token balance before generation
    console.log(`💳 Attempting to deduct ${tokenCost} tokens for user ${userId.substring(0, 8)}...`)

    try {
      const deductionResult = await deductTokens(userId, tokenCost, `Image generation (img2img)`, {}, "image_generation")
      if (!deductionResult) {
        console.error("❌ Token deduction failed")
        return NextResponse.json({
          error: "Insufficient tokens",
          insufficientTokens: true,
          currentBalance: 0, // We don't have the exact balance here, but the frontend will handle it
