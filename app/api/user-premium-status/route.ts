...
export async function GET(request: NextRequest) {
    ...
    // Note: In some cases (server-side calls), we might trust the request if checking specific conditions
    // But generally we want to ensure the user is checking their own status
    if (!authenticatedUserId || authenticatedUserId !== userId) {
        // If we can't authenticate via headers, try checking if the session cookie belongs to the user
        const supabase = createClientWithCookies(cookieStore)
        ...
        if (sessionUser && sessionUser.id === userId) {
            ...
            console.log(`⚠️ Unauthorized check for ${userId}. Auth user: ${authenticatedUserId}, Session user: ${sessionUser?.id}`)
    ...
    try {
        ...
        try {
            ...
            if (!profileError && profile) {
                console.log("✅ Found profile status:", profile);
                if (profile.is_premium) {
                    ...
                    if (expiresAt) {
                        ...
                        if (!isPremium) {
                            console.log(`⚠️ Profile says premium but expired at ${expiresAt}`);
                        }
                    } else {
                        isPremium = true; // Permanent premium if no expiry?
                    }
                    
                    if (isPremium) {
                        console.log(`✅ Confirmed premium from profiles table. Expires: ${expiresAt}`);
                        // We don't get plan name from profiles usually, but that's okay
        ...
        // Method 1: Check premium_profiles table (secondary location)
        if (!isPremium) {
            try {
                ...
                if (!profileError && premiumProfile) {
                    ...
                    const planData = Array.isArray(premiumProfile.subscription_plans)
                        ? premiumProfile.subscription_plans[0]
                    ...
                    if (profileError?.message?.includes("relationship")) {
                        console.log("⚠️ Schema relationship error in premium_profiles (ignoring)");
                    } else {
                        console.log("❌ No premium profile found:", profileError?.message);
                    }
                }
            } catch (profileError) {
                console.log("❌ premium_profiles query failed (likely schema issue):", profileError);
            }
        }
        ...
        if (!isPremium) {
            ...
            try {
                ...
                try {
                    ...
                    console.log("⚠️ Failed to join payment_transactions with subscription_plans");
                    paymentError = { message: "Join failed" };
                }
                ...
                if (!paymentError && recentPayments) {
                    ...
                    if (recentPayments.metadata && recentPayments.metadata.planDuration) {
                        ...
                        const planData = Array.isArray(recentPayments.subscription_plans)
                            ? recentPayments.subscription_plans[0]
