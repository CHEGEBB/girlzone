...
export default function EditCollectionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchCollection = async () => {
    setIsLoading(true)
    try {
      const result = await getCollection(resolvedParams.id)
      if (result.success) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result = await updateExistingCollection(resolvedParams.id, formData)
      ...
      if (result.success) {
        ...
        router.push(`/collections/${resolvedParams.id}`)
      } else {
        throw new Error(result.error)
      }
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <Button
        variant="outline"
        size="sm"
        onClick={() => router.push(`/collections/${resolvedParams.id}`)}
      ...
      {isLoading ? (
          ...
          <CardContent className="p-6">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex justify-end gap-4">
                ...
                <Button type="submit" disabled={isSaving} className="bg-[#FF4D8D] hover:bg-[#FF3D7D]">
        ...
        <div className="text-center">
          ...
          <Button onClick={() => router.push("/collections")} className="bg-[#FF4D8D] hover:bg-[#FF3D7D]">
