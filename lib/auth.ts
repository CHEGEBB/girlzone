...
export async function signUp(email: string, password: string) {
  const { data, error } = await getSupabase().auth.signUp({
    ...
    password,
    options: {
      emailRedirectTo: typeof window !== 'undefined' ? `${window.location.origin}/admin/login` : undefined,
    },
  })
...
export async function isAdmin(userId: string) {
  try {
    ...
    // First check if we have a cached result (client-side only)
...
export async function createMockAdminUser() {
  ...
  const { data: existingUser } = await getSupabase().auth.signInWithPassword({
    email,
    password,
  })
  ...
  if (existingUser?.user) {
    ...
    try {
      const { data: adminData, error } = await getSupabase()
        .from("admin_users")
        .select("*")
      ...
      if (!adminData && !error) {
        ...
        await getSupabase().from("admin_users").insert([{ user_id: existingUser.user.id }])
      }
    } catch (error) {
  ...
  // Create the user
  const { data: newUser, error: signUpError } = await getSupabase().auth.signUp({
    email,
    password,
  })
  ...
  try {
    // Add to admin_users table
    await getSupabase().from("admin_users").insert([{ user_id: newUser.user.id }])
  } catch (error) {
