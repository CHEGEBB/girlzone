...
// Replace all messages in localStorage for a character
export function replaceMessagesInLocalStorage(characterId: string, messages: Message[]): boolean {
  if (typeof window === "undefined") {
    return false
  }
  try {
    ...
    console.log(`All messages replaced in localStorage for character ${characterId}`)
    return true
  } catch (error) {
    console.error("Error replacing messages in localStorage:", error)
    return false
  }
}
