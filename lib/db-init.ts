...
video_url TEXT,
disable_video_hover BOOLEAN DEFAULT FALSE,
...
export async function getApiKey(key: string): Promise<string | null> {
  try {
    ...
    const { data, error } = await (supabase.from("app_settings").select("value").eq("id", key) as any).maybeSingle()
...
export async function setApiKey(key: string, value: string): Promise<boolean> {
  try {
    const { error } = await (supabase
      .from("app_settings")
      .upsert({ id: key, value, updated_at: new Date().toISOString() } as any) as any)
      .select()
