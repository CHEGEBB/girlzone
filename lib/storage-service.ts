...
export class StorageService {
  // Get user ID (either authenticated or anonymous) - client-side
  ...
  // Get user ID (either authenticated or anonymous) - server-side
  static async getUserIdServer() {
  ...
  static async getCharacter(id: string) {
    const supabaseAdmin = await getAdminClient()
    const userId = await this.getUserIdServer() // Use server-side method

    // Try to get character with user_id check first, then fallback to public characters
    const { data, error } = await supabaseAdmin
      .from("characters")
      .select("*")
      .eq("id", id)
    ...
    if (data.user_id && data.user_id !== userId && !data.is_public) {
  ...
  static async getPrompts(characterId?: string) {
    const supabaseAdmin = await getAdminClient()
    const userId = await this.getUserIdServer()
