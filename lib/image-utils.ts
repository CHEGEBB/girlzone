...
export const isAskingForImage = (message: string): boolean => {
  ...
  // Don't trigger image if asking for video
  if (isAskingForVideo(message)) return false
  return (
...
/**
 * Checks if a message is asking for video generation.
 * @param message The message to check.
 * @returns True if the message asks for a video, false otherwise.
 */
export const isAskingForVideo = (message: string): boolean => {
  if (!message) return false
  const lowerCaseMessage = message.toLowerCase()
  return (
    lowerCaseMessage.includes("video") ||
    lowerCaseMessage.includes("clip") ||
    lowerCaseMessage.includes("animate") ||
    lowerCaseMessage.includes("in motion") ||
    lowerCaseMessage.includes("moving")
  )
}
